    ...
    <div className="relative">
      ...
      <div className="relative mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-5">
          {/* Trending memes */}
          <div className="rounded-[28px] border border-navy-900/8 bg-white p-5 shadow-card lg:col-span-3">
            <h2 className="font-display text-lg font-bold text-navy-900">
              Trending memes · last 24 hours
            </h2>
            <ol className="mt-4 flex flex-col divide-y divide-navy-900/8">
              {trending.map((m, i) => (
                <li key={m.id} className="flex items-center gap-3 py-3">
                  <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-violet-500 to-pink-500 text-sm font-bold text-white">
                    {i + 1}
                  </span>
                  <div className="min-w-0 flex-1">
                    <Link
                      href={`/meme/${m.slug}`}
                      className="truncate font-bold text-navy-900 hover:text-violet-700"
                    >
                      {m.title}
                    </Link>
          ...
          <div className="rounded-[28px] border border-navy-900/8 bg-white p-5 shadow-card lg:col-span-2">
        ...
        <div className="mt-6 overflow-hidden rounded-[28px] border border-navy-900/8 bg-white shadow-card">
          ...
          <div className="overflow-x-auto">
            <table className="mt-4 w-full min-w-[520px] text-left text-sm">
              <thead>
                <tr className="border-y border-navy-900/8 text-xs font-bold tracking-wide text-navy-500 uppercase">
              ...
              <tbody className="divide-y divide-navy-900/8">
                {leaderboard.map((m) => (
                  <tr key={m.id} className="transition hover:bg-violet-50/50">
                    <td className="px-5 py-3">
                      <Link href={`/meme/${m.slug}`} className="font-bold text-navy-900 hover:text-violet-700">
                        {m.title}
                      </Link>
                    </td>
                    ...
                    <td className="px-5 py-3">
                      <span className="rounded-full bg-gradient-to-r from-violet-100 to-pink-100 px-2.5 py-1 text-xs font-extrabold text-violet-800">
