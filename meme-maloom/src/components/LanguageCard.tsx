...
export default function LanguageCard({ info }: { info: LanguageInfo }) {
  ...
  return (
    <Link
      ...
      className="group flex flex-col gap-3 rounded-3xl border-2 border-navy-900/10 bg-white p-5 shadow-card transition hover:-translate-y-1 hover:border-saffron-400 hover:shadow-card-hover"
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="font-display text-2xl font-extrabold text-navy-900">{info.nativeName}</p>
          <p className="text-sm font-bold text-saffron-600">{info.language}</p>
        </div>
      ...
      <div className="mt-1 rounded-2xl bg-navy-900/5 p-3">
      ...
      <span className="mt-1 text-sm font-bold text-saffron-600 group-hover:underline">
